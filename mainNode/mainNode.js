...
const argv = yargs
	...
	.option('config', {
		...
		type: 'String',
		default: 'mainNode-config_1.json'
	})
	.option('ize', {
...
const address = config.mainNode.address;
const key = config.mainNode.private_key;
...
async function get_all(func_sig) {
	...
	for (tx of txs.result) {
		if (tx.input.slice(2, 10) == func_sig && tx.isError == '0') {
...
if (argv.a) {
	address_to_add = config.mainNode_to_add.address;
	...
	instance.methods.getBlockID().call((err, blk_id) => {
		...
		web3.eth.getTransactionCount(address).then(txCount => {
			...
			//	.on('receipt', console.log)
		    	.on('transactionHash', txhash => {console.log("[Transaction Success]: ", txhash);})
		    	.on('error', error => {console.log("[Transaction Fail]");});
    	})
	})
}
...
if (argv.t) {
/*	api.account.txlist(contract_address, 1, 'latest', 1, 100, 'asc').then(txs => {
		for (tx of txs.result) {
			if (tx.input.slice(2, 10) == "ae8d0145" && tx.isError == '0') {
				console.log(tx);
			}
		}
	})*/
	instance.methods.getBlockID().call((err, blk_id) => {
		const block = "hello";//receiveBlock(argv.size);
	//	process.stdout.write(argv.size.toString()+ ',');
		const hash = web3.utils.soliditySha3({t: "string", v: block});
		let sig = web3.eth.accounts.sign(hash, key);
		console.log("hash = ", sig);

    //web3.eth.sign(hash1, addr1);
	/*	const new_blk_id = parseInt(blk_id) + 1;
		const functionABI = instance.methods.sendBlock(block, sig.signature, new_blk_id).encodeABI();
		web3.eth.getTransactionCount(address).then(txCount => {
			const txData = {
	 		...
	 		web3.eth.estimateGas(txData).then(console.log);
		})*/
	})
