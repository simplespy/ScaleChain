...
# Seattle Sill Amsterdam Tokyo Chicago Paris London Dallas Singapore New-Jersey 
declare -a regions=(4 12 7 25 2 24 8 3 40 1)
regions_len=${#regions[@]}
bin="../target/debug/system_rust"

plan=203 # 10 dollar per month =  1cpu 2G ram
os=270   # ubuntu os
...
{
	ssh $1 -- "rm -rf /root/payload && rm -f /root/*.tar.gz && wget ${s3_prefix}/$1.tar.gz -O /root/local.tar.gz && wget ${s3_prefix}/common.tar.gz -O /root/common.tar.gz && mkdir -p /root/payload && tar xf /root/local.tar.gz -C /root/payload/ && tar xf /root/common.tar.gz -C /root/payload/"
}
...
{
	echo "$3 $4"
	curl "http://$3:$4/transaction-generator/start?interval=14000"
}
